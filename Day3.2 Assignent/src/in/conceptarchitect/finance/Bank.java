...
private String salt(String password) {
	...
	public void transferTo(int account,double amount, String password, BankAccount target){ 
			if(account==accountNumber) {
			
		if(BA.withdraw( amount, password)) { 
		target.deposit(amount);
		} 
		else {
			System.out.println("Enter Valid amount or password or target account");
		}
		}else {
	...
	public void Close(int account) {
		if(account==accountNumber) {
			System.out.println("Account Dlected");
		}else {
			System.out.println("Enter Valid Account");
		}
	}
